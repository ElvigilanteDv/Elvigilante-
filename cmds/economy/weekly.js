export default {
  ...
  run: async ({ msg, sock, usedPrefix }) => {
    ...
    if (!global.db.data.users[senderId]) global.db.data.users[senderId] = {};
    if (!global.db.data.chats[chatId]?.users?.[senderId]) {
      global.db.data.chats[chatId].users[senderId] = {};
    }
    
    global.db.data.users[senderId].weeklyStreak ??= 0;
    global.db.data.users[senderId].lastWeeklyGlobal ??= 0;
    global.db.data.chats[chatId].users[senderId].lastweekly ??= 0;
    
    const users = global.db.data.users[senderId];
    ...
    if (now < user.lastweekly) {
      ...
      return msg.reply(`🐉🌀 Ya has reclamado tu recompensa semanal.\n⚡ Puedes reclamarlo de nuevo en *${wait}*`);
    }
    ...
    if (canClaimWeeklyGlobal) {
      ...
      global.db.data.users[senderId].weeklyStreak = currentStreak;
      global.db.data.users[senderId].lastWeeklyGlobal = now;
    }
