...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      if (args[0] === '-list') {
        let helpText = `🐉 *GOTENKS V1 STICKER* 🌀
...
⚡ Ejemplo: *${usedPrefix + command} -c -blur Pack | Autor*`;
      ...
      if (/image/.test(mime) || /webp/.test(mime)) {
        ...
        if ((quoted.msg || quoted).seconds > 20) {
          return m.reply('🐉🌀 El video no puede ser muy largo (máx 20 segundos).');
        }
        let buffer = await quoted.download();
        ...
        if (!url.match(/\.(jpe?g|png|gif|webp|mp4|mov|avi|mkv|webm)(\?.*)?$/i)) {
          return m.reply('🐉🌀 La URL debe ser de una imagen o video.');
        }
        const response = await fetch(url);
        if (!response.ok) return m.reply('🐉🌀 No pude descargar ese archivo desde la URL.');
        ...
        return m.reply(`🐉🌀 Por favor, envía una imagen, video, sticker o URL.\n⚡ Usa *${usedPrefix + command} -list* para ver formas y efectos`);
      }
    } catch (e) {
      return m.reply(`🐉🌀 Error al ejecutar el comando *${usedPrefix + command}*.\n⚡ [Error: *${e.message}*]`);
    }
  }
};
